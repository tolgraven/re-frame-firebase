...
(ns com.degel.re-frame-firebase.database
  (:require
   ...
   ["firebase/database" :as firebase-database :refer [getDatabase ref set update push onValue off]]
...
(defn once-effect [{:keys [path on-success on-failure]}]
  ;; In v9+, once is replaced with get() from database
  (.once (fb-ref path)
         "value"
...
(defn on-value-sub [app-db [_ {:keys [path on-failure]}]]
  (if path
    (let [ref (fb-ref path)
      ...
      (onValue ref callback (event->fn (or on-failure (core/default-error-handler))))
      (make-reaction
